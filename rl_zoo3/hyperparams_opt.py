...
def sample_sac_params(trial: optuna.Trial, n_actions: int, n_envs: int, additional_args: dict) -> Dict[str, Any]:
    ...
    net_arch_type = trial.suggest_categorical(
        "net_arch", ["f10_small", "f10_medium", "f10_large", "f10_verylarge", "f10_3_large"]
    )
    ...
    net_arch = {
        ...
        "f10_medium": [800, 600],
        "f10_large": [1600, 1200],
        "f10_verylarge": [3200, 2400],
        "f10_3_large": [1600, 1200, 800],
    }[net_arch_type]
    ...
    hyperparams = {
        ...
        "policy_kwargs": dict(log_std_init=log_std_init, net_arch=net_arch, use_expln=use_expln),
    }
...
def sample_td3_params(trial: optuna.Trial, n_actions: int, n_envs: int, additional_args: dict) -> Dict[str, Any]:
    ...
    policy_delay = trial.suggest_categorical("policy_delay", [1, 2, 4, 8, 16, 48, 128, 512])

    # NOTE: Add "verybig" to net_arch when tuning HER
    # net_arch_type = trial.suggest_categorical("net_arch", ["small", "medium", "big"])
    net_arch_type = trial.suggest_categorical("net_arch", ["big", "huge", "giant", "massive"])
    # activation_fn = trial.suggest_categorical('activation_fn', [nn.Tanh, nn.ReLU, nn.ELU, nn.LeakyReLU])

    target_policy_noise = trial.suggest_float("target_policy_noise", 0, 0.5)
    target_noise_clip = trial.suggest_float("target_noise_clip", 0, 0.75)
...
def sample_crossq_params(trial: optuna.Trial, n_actions: int, n_envs: int, additional_args: dict) -> Dict[str, Any]:
    ...
    hyperparams["policy_kwargs"].update({"batch_norm_momentum": batch_norm_momentum, "batch_norm_eps": batch_norm_eps})
    return hyperparams
